...
* The `data_id` aesthetic can be used to link up data.
    * 1:1 single points
    * 1:many highlight groups
    * many:1 single datum represented multiple times
* It can be used to link across composed plots.
...
* In `girafe(options)` you can customise the following:
    * Tooltips via `opts_tooltip()`
    * Hovering via `opts_hover()`/`opts_hover_inv()`
    * Zooming via `opts_zoom()`
    * Selections via `opts_selection()`/`opts_selection_inv()`
* The `onclick` property can be used to link execute JavaScript.
